...
const REPORT_HISTORY_PATH = path.resolve(__dirname, '../../../../99_Sbox/ルート日報/Report_Sync.md');
...
if (GEMINI_API_KEY) {
    ...
    global.model = genAI.getGenerativeModel({ model: "gemini-1.5-flash-latest" });
}
...
app.get('/api/customers', async (req, res) => {
    try {
        ...
        // Return only the customers part if it exists, otherwise the whole object
...
app.post('/api/voice-process', upload.single('audio'), async (req, res) => {
    ...
    try {
        ...
        // No need to load customer list since we no longer extract customer names
